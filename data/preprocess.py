...
# --- to do with loading --- #
# --- to do with loading --- #
def get_samples_and_labels(config):
    """
...
############################################ data-specific preprocessing functions ############################################
def swat(seq_length, seq_step, num_signals, randomize=False):
    """ Load and serialise """
    train = np.loadtxt(open('swat/train.csv'), delimiter=',')
...
def swat_test(seq_length, seq_step, num_signals, randomize=False):
    ...
    test = np.loadtxt(open('swat/test.csv'), delimiter=',')
    print('Loaded swat test from .csv')
